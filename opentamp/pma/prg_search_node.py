...
class HLSearchNode(SearchNode):
    ...
    def plan(self, solver, debug=False):
        plan_obj = solver.solve(self.abs_prob, 
                                self.domain, 
                                self.concr_prob, 
                                self.prefix, 
                                label=self.label,
...
class LLSearchNode(SearchNode):
    ...
    def parse_state(self, plan, failed_preds, ts, all_preds=[]):
        # Initialize the ne wlogical state around the provided list
        ...
        for a in plan.actions:
            ...
            # all_preds should be added to each action preds
            ...
            for p in preds:
                ...
                if p['pred'].get_rep() in reps: continue

                # Remember that this pred has been checked already
                reps.append(p['pred'].get_rep())
                st, et = p['active_timesteps']

                # hl_include means it will always be added
                if p['pred'].hl_include: 
                    new_preds.append(p['pred'])
                    continue

                # hl_ignore means it will never be added
                if p['pred'].hl_ignore:
                    continue
    ...
    def get_problem(self, i, failed_pred, failed_negated, suggester=None):
        ...
        if not len(self.curr_plan.actions) or self.curr_plan.actions[-1].active_timesteps[1] < i:
            ...
            act_enum = enumerate(self.curr_plan.actions)
            anum, last_action = [(a_ind, a) 
                                  for a_ind, a in act_enum 
                                  if a.active_timesteps[0] <= i and a.active_timesteps[1] >= i
                                 ][0]
    ...
    def solved(self):
        if self._solved is None:
            return len(self.curr_plan.get_failed_preds(tol=self.tol)) == 0
        return self._solved

    def is_ll_node(self):
        return True

    def gen_plan(self, hl_solver, ll_solver):
        if self.plan_str is None:
            raise ValueError('No plan_str to gen plan from!')

        self.curr_plan = hl_solver.get_plan(self.plan_str, 
                                            self.domain, 
                                            self.concr_prob, 
                                            self.initial)
        ...
        if not len(self.curr_plan.actions):
            print('Search node found bad plan for:', 
                  self.initial, 
                  self.plan_str, 
                  self.concr_prob.goal)
