...
# solvers.
domain_fname = opentamp.__path__[0] + "/domains/robot_wiping_domain/right_wipe_onlytable.domain"
prob = opentamp.__path__[0] + "/domains/robot_wiping_domain/probs/simple_move_onlytable_prob.prob"
...
# pdb.set_trace()
body_ind = env.sim.model.body_name2id("robot0_base")
table_ind = env.sim.model.body_name2id("table")
...
sawyer_inds = []
for jnt in jnts:
    jnt_adr = env.sim.model.joint_name2id(jnt)
    jnt_ind = env.sim.model.jnt_qposadr[jnt_adr]
...
grip_ind = env.sim.model.site_name2id("gripper0_grip_site")
hand_ind = env.sim.model.body_name2id("robot0_right_hand")
        ...
        if ctrl_mode.find("JOINT") >= 0:
            ...
            for n in range(nsteps):
                ...
                if render_t == 0:
                    gif_frames.append(
                            Image.fromarray(
                                env.sim.render(height=64, width=64, camera_name="frontview")
                            )
                    )
