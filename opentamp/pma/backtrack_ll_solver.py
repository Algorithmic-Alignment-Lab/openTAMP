...
from sco_py.sco_gurobi.prob import Prob as GurobiProb
from sco_py.sco_gurobi.solver import Solver as GurobiSolver
from sco_py.sco_gurobi.variable import Variable as GurobiVariable

from sco_py.sco_osqp.prob import Prob as OSQPProb
from sco_py.sco_osqp.solver import Solver as OSQPSolver
from sco_py.sco_osqp.variable import Variable as OSQPVariable
...
from opentamp.pma.ll_solver_gurobi import LLParam as LLParamGurobi, LLSolver as LLSolverGurobi
...
def solve_helper(solver_class, plan, callback, active_ts, verbose):
    # certain constraints should be solved first
    success = False
    for priority in solver_class.solve_priorities:
        success = solver_class._solve_opt_prob(plan, priority=priority,
                        callback=callback, active_ts=active_ts,
                        verbose=verbose)
    return success


class BacktrackLLSolverGurobi(LLSolverGurobi):
    ...
    def _solve_helper(self, plan, callback, active_ts, verbose):
        solve_helper(self, plan, callback, active_ts, verbose)
    ...
    def _solve_opt_prob(self, plan, priority, callback=None, init=True,
        ...
        self._prob = GurobiProb(model, callback=callback)
    ...
    def create_variable(self, grb_vars, init_vals, save=False):
        ...
        if sco_var is None:
            sco_var = GurobiVariable(grb_vars, np.array(ret_val).reshape((len(ret_val), 1)))
...
class BacktrackLLSolverOSQP(LLSolverOSQP):
    ...
    def _solve_opt_prob(
        ...
        self._prob = OSQPProb(callback=callback)
    ...
    def create_variable(self, osqp_vars, init_vals, save=False):
        ...
        if sco_var is None:
            sco_var = OSQPVariable(osqp_vars, np.array(ret_val).reshape((len(ret_val), 1)))
            self.var_list.append(sco_var)
