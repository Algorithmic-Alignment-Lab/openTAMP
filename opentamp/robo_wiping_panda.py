...
params["panda"].pose[:, 0] = env.sim.data.body_xpos[body_ind]
...
jnts = params["panda"].geom.jnt_names["right"]
...
for jnt in jnts:
    ...
    panda_inds.append(jnt_ind)
    jnt_vals.append(env.sim.data.qpos[jnt_ind])
params["panda"].right[:, 0] = jnt_vals
params["robot_init_pose"].right[:, 0] = jnt_vals
params["robot_init_pose"].value[:, 0] = params["panda"].pose[:, 0]
params["panda"].openrave_body.set_pose(params["panda"].pose[:, 0])
params["panda"].openrave_body.set_dof({"right": params["panda"].right[:, 0]})
info = params["panda"].openrave_body.fwd_kinematics("right")
params["panda"].right_ee_pos[:, 0] = info["pos"]
params["panda"].right_ee_pos[:, 0] = T.quaternion_to_euler(info["quat"], "xyzw")
...
#                 print(f'("region_pose{row}_{col}", np.{repr(params["panda"].openrave_body.get_ik_from_pose(xyz_pose, quat, "right"))}),')
# exit()
...
for t in range(plan.horizon):
    rgrip = panda.right_gripper[0, t]
    if true_mode.find("JOINT") >= 0:
        act = np.r_[panda.right[:, t]]
    else:
        pos, euler = panda.right_ee_pos[:, t], panda.right_ee_rot[:, t]
...
env.sim.data.qpos[:7] = params["panda"].right[:, 0]
env.sim.data.qacc[:] = 0
env.sim.data.qvel[:] = 0
env.sim.forward()
rot_ref = T.euler_to_quaternion(params["panda"].right_ee_rot[:, 0], "xyzw")

for _ in range(40):
    env.step(np.zeros(7))
    env.sim.data.qpos[:7] = params["panda"].right[:, 0]  # This will help set the simulator joint sets!
    env.sim.forward()
...
true_lb, true_ub = plan.params["panda"].geom.get_joint_limits("right")
...
for act in plan.actions:
    ...
    plan.params["panda"].right[:, t] = env.sim.data.qpos[:7]
    ...
    for t in range(act.active_timesteps[0], act.active_timesteps[1]):
        ...
        if ctrl_mode.find("JOINT") >= 0 and true_mode.find("JOINT") < 0:
            ...
            if t < plan.horizon:
                targ_pos, targ_rot = (
                    panda.right_ee_pos[:, t + 1],
                    panda.right_ee_rot[:, t + 1],
                )
            else:
                targ_pos, targ_rot = (
                    panda.right_ee_pos[:, t],
                    panda.right_ee_rot[:, t],
                )
            lb = env.sim.data.qpos[:7] - factor
            ub = env.sim.data.qpos[:7] + factor
            panda.openrave_body.set_dof({"right": np.zeros(7)})
            panda.openrave_body.set_dof({"right": ref_jnts})

            targ_jnts = panda.openrave_body.get_ik_from_pose(
        ...
        if ctrl_mode.find("JOINT") >= 0:
            ...
            ee_to_sim_discrepancy = (
                env.sim.data.site_xpos[grip_ind] - panda.right_ee_pos[:, t]
            )
